# J3Plugins D17 Submission Plan

## Goal

Keep `J3Plugins` alive as the custom Dalamud plugin repository while moving each plugin into a dedicated source repository. This keeps custom repo installs working, gives each plugin a clean review surface, and lets official DalamudPluginsD17 submissions point directly at the plugin repo that owns the source.
...
J3Plugins/
  ...
  repo.json
  src/
    CutsceneTranscripts/      -> git submodule: J3sven/CutsceneTranscripts
    CursorIndicator/          -> git submodule: J3sven/CursorIndicator
    IINACTNativeOverlay/      -> git submodule: J3sven/IINACTNativeOverlay
    Chocobot/                 -> git submodule: J3sven/Chocobot
```

Each dedicated plugin repo should have the plugin project at its repository root:

```text
CutsceneTranscripts/
  CutsceneTranscripts.csproj
  CutsceneTranscripts.json
  packages.lock.json
  Assets/
  README.md
```
...
- Plugin source of truth: the dedicated plugin repo.
- Custom repo source of truth: `repo.json`, submodule commit pins, custom repository docs, and release links.
- DalamudPluginsD17 source of truth: an exact commit hash from the dedicated plugin repo.

## Migration Plan

1. Split `src/CutsceneTranscripts` into `J3sven/CutsceneTranscripts` first, since it is the D17 candidate.
2. Verify the dedicated plugin repo builds on its own with `dotnet build -c Release`.
3. Replace `src/CutsceneTranscripts` in `J3Plugins` with a git submodule pinned to the verified plugin repo commit.
4. Keep `repo.json` in `J3Plugins` working for the custom repository. Release links can remain on `J3Plugins` initially, or move to each dedicated plugin repo later.
5. Submit Cutscene Transcripts to DalamudPluginsD17 from the dedicated repo, not through the `J3Plugins` umbrella repo.
6. Repeat the split for the other plugins once the Cutscene Transcripts workflow is proven.

## D17 Manifest Shape

Use `testing/live` for the initial submission:

```toml
[plugin]
repository = "https://github.com/J3sven/CutsceneTranscripts.git"
commit = "<exact plugin repo commit>"
owners = ["J3sven"]
maintainers = ["J3sven"]
project_path = "."
...
- Refactor regular transcript/settings windows to use `Dalamud.Interface.Windowing`.
- Do a human review pass over unsafe addon reads, choice capture, voice capture, and voice replay behavior.
- Manually test common cutscene flows before opening the PR.
- Disclose AI involvement in the PR. Current expected disclosure level is `Copilot` if the human plan/review/testing drove the work and Codex wrote most code, or `Auto` if Codex mostly designed and implemented with minimal direction.
- Disclose any AI-generated user-facing assets if applicable.

---

# DalamudPluginsD17 Reference
